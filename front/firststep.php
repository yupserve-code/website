...
elseif (isset($_POST['action']) && $_POST['action'] == 'check_user_email')
	...
	if (mysqli_num_rows($check_user_mail) > 0)
        ...
        elseif (isset($_POST['action']) && $_POST['action'] == 'check_user_email_exists')
	{
	$email = trim(strip_tags(mysqli_real_escape_string($conn, $_POST['email'])));
	$query="SELECT * FROM ct_users WHERE `user_email`='" . $email . "' AND `status`='E'";
        $email_user = mysqli_query($conn, $query);
	if (mysqli_num_rows($email_user) > 0)
        ...
        elseif (isset($_POST['action']) && $_POST['action'] == 'check_user_phone')
	{
	$phone = trim(strip_tags(mysqli_real_escape_string($conn, $_POST['phone'])));
        $mobile_phone = "+91" . $phone;
	$query="SELECT * FROM ct_users WHERE `phone`='" . $mobile_phone . "' AND `status`='E'";
        $phone_user = mysqli_query($conn, $query);
	if (mysqli_num_rows($phone_user) > 0)
		{
