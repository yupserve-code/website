...
class Booking_Details {
    ...
    public function index() {
        ...
        if (!empty($_SERVER['REQUEST_METHOD']) && ($_SERVER['REQUEST_METHOD'] == "GET")) {
            ...
            if (!empty($_GET['order_id']) && !empty($_GET['staff_id'])) {
			...
			*/
                    ...
                    $booking = new cleanto_staff_commision();
                    $booking->conn = $conn;
					
					$booking_2 = new cleanto_booking();
                    $booking_2->conn = $conn;

                    $order_result = $booking->get_staff_app_booking_customer_details($order_id);
                    ...
                    if (mysqli_num_rows($order_result) > 0) {
                        ...
                        // Task status
                        $otp_result = $booking->get_booking_otp($order_id);
                        
                        if (mysqli_num_rows($otp_result) > 0) {
                            $row_otp = mysqli_fetch_object($otp_result);
                            if ($row_otp->verified == 1) {
                                $tsk_started = TRUE;
                            } else {
                                $tsk_started = FALSE;
                            }
                        ...
                        // Service details
                        $service_results = $booking->get_staff_service_addon_bookings($order_id);
                        ...
                        $invoice_results = $booking_2->get_invoice_generated($order_id);
                        
                        if (mysqli_num_rows($invoice_results) > 0) {
                            $payment_query = "SELECT SUM(amount) AS amount FROM `ct_proforma_invoice` WHERE `order_id`={$order_id} AND is_active = 0";
                            $payment_total = mysqli_query($conn, $payment_query);
                            //$payment_total = $booking_2->get_app_invoice_details($order_id);
                        ...
                        if (mysqli_num_rows($payment_results) > 0) {
							...
							$sql_inv1 = "SELECT * FROM ct_invoice WHERE order_id='{$order_id}' AND is_active = 0 ";
