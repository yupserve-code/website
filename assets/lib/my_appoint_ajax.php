...
include(dirname(dirname(dirname(__FILE__))).'/assets/lib/Firebase.php');
...
elseif(isset($_POST['confirm_booking_cal'])){
    ...
    $message = "Booking has been confirmed for order #{$id}";
    date_default_timezone_set('Asia/Kolkata');
    $current_time = date('Y-m-d H:i:s', time());
    
    $sql_ord = "SELECT * FROM ct_bookings WHERE order_id='{$id}'";
    $res_ord = mysqli_query($conn, $sql_ord);
    $row_order = mysqli_fetch_object($res_ord);
    
// Get the customer Device Token
    $sql_token = "SELECT token FROM ct_customer_device_token WHERE customer_id='{$row_order->client_id}'";
...
// data array to be sent to firebase server
    ...
    $sql_ins_notif = "INSERT INTO ct_user_notification (user_id, order_id, text, is_read, created_at) VALUES('{$row_order->client_id}', '{$id}', '{$message}', '0', '{$current_time}')";
...
elseif(isset($_POST['getallnotification'])){
    ...
    $sql_noti = "SELECT b.*, n.id AS notification_id, n.created_at, n.status, n.is_read FROM ct_bookings b JOIN ct_booking_task_history n ON b.order_id = n.order_id WHERE n.is_read=0 ORDER BY n.created_at DESC";
    $books = mysqli_query($conn, $sql_noti);
    while($b = mysqli_fetch_array($books)) {
		...
		?>
            <a id="<?php echo $b['notification_id'];?>" href="javascript:void(0);" class="admin_notification_click" val="<?php echo $b['order_id']; ?>">
                <input type="hidden" name="order_id" id="order_id" class="order_id" value="<?php echo $b['order_id']; ?>"/>
		<li id="rec-noti-1" class="notificationli" style="background-color: <?php echo $col;?>">
			<div class="list-inner">
				...
				if($b['client_id']==0)
				{
					...
					?>
					<span class="booking-text"><?php echo $booking_stats;?> <?php  echo $gc[2]." ".$label_language_values['for_a']." ".$b['status']." ".$label_language_values['on']." ".str_replace($english_date_array,$selected_lang_label,date($getdateformate, strtotime($b['booking_date_time'])));?> @ <?php 
					if($time_format == 12){
					?>
				...
				else
				{
					...
					?>
					<span class="booking-text"><?php echo $booking_stats;?> <?php  echo $b['status']." ".$label_language_values['on']." ";?><?php echo str_replace($english_date_array,$selected_lang_label,date($getdateformate, strtotime($b['created_at'])));?> @ <?php 
					if($time_format == 12){
					?>
					<?php echo str_replace($english_date_array,$selected_lang_label,date("h:i A",strtotime($b['booking_date_time'])));?></span>
					<?php } else { ?>
					<?php  echo date("H:i", strtotime($b['booking_date_time']));?></span>
					<?php } ?></span>
					<span class="booking-time"><?php echo time_elapsed_string($b['lastmodify']); ?></span>
				<?php } ?>
			</div>
		</li>
		</a>
...
elseif(isset($_POST['reject_booking'])){
    ...
    $message = "Booking has been rejected by Yupserve Team for order #{$id}";
    date_default_timezone_set('Asia/Kolkata');
    $current_time = date('Y-m-d H:i:s', time());
...
// data array to be sent to firebase server
	...
	if ($row_order->staff_ids != '' || $row_order->staff_ids != NULL) {
		$staff_id = $row_order->staff_ids;
		
		// Get the staff Device Token
		$sql_staff_token = "SELECT token FROM ct_staff_device_token WHERE staff_id='{$row_order->staff_ids}'";
		$res_staff_token = mysqli_query($conn, $sql_staff_token);
		$row_staff_token = '';
		if (mysqli_num_rows($res_staff_token) > 0) {
	...
	// data array to be sent to firebase server
		...
		// send push notification
		$firebase->send_to_single($row_staff_token->token, $dataStaffs, $notifStaff_array);
		
		// INSERT into staff notification table
		$sql_ins_staff_notif = "INSERT INTO ct_staff_notification (staff_id, order_id, text, is_read, created_at) VALUES('{$row_order->staff_ids}', '{$id}', '{$message}', '0', '{$current_time}')";
		mysqli_query($conn, $sql_ins_staff_notifs);
     ...
     */
    $stat_msg = "Task has been rejected by Yupserve Team for order #{$id}";
    $sql_ins_stat = "INSERT INTO ct_booking_task_history (order_id, staff_id, customer_id, status, created_at) VALUES ('{$id}', '{$staff_id}', '{$row_order->client_id}', '{$stat_msg}', '{$current_time}')";
...
?>
<?php 
if (isset($_POST['action']) && ($_POST['action'] == 'admin_notification_click')) {
    $json = array();
    $notif_id = $_POST['id'];
    $sql_update = "UPDATE ct_booking_task_history SET is_read=1 WHERE id='{$notif_id}'";
