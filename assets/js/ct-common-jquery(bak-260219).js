...
jQuery(document).on('click', '#login_existing_user', function () {
    ...
    dataString = {existing_username: existing_username, action: "get_existing_user_data"};
    ...
    jQuery.ajax({
        ...
        success: function (response) {
            ...
            if (userdata.error) {
                ...
                jQuery('.login_unsuccessfull').html(userdata.error);
            } else if (userdata.success) {
                // display the OTP input field
                $('.ct-login-otp').show();
                $('#login_existing_user').hide();
                $('hide_password').show();
...
jQuery(document).on('click', '#ct_login_otp', function () {
    jQuery('.add_show_error_class_for_login').each(function () {
        jQuery(this).trigger('keyup');
    });
    jQuery('.add_show_error_class').each(function () {
    ...
    var site_url = siteurlObj.site_url;
    var ajax_url = ajaxurlObj.ajax_url;
    var otp = jQuery("#ct-user-otp").val();
    //dataString = {existing_username: existing_username, existing_password: existing_password, action: "get_existing_user_data"};
    dataString = {otp: otp, action: "verify_user_otp"};

    if (!jQuery('#ct-user-otp').valid()) {
        return false;
    }
    ...
    jQuery.ajax({
        ...
        success: function (response) {
            ...
            if (userdata.error) {
                ...
                jQuery('.otp_error').css('color', 'red');
                jQuery('.otp_error').css('margin-left', '15px');
                jQuery('#check_login_click').val('not');
                //jQuery('.login_unsuccessfull').html(errorobj_incorrect_email_address_or_password);
                jQuery('.otp_error').html(userdata.error);
            } else if (userdata.success) {
                // display the OTP input field
                $('.ct-login-otp').hide();
                $('#ct_login_otp').hide();
...
jQuery(document).on('click', '#ct-user-otp', function () {
    jQuery('.otp_error').hide();
});
jQuery(document).ready(function () {
    ...
    jQuery('#user_login_form').validate({
        rules: {
            //ct_user_name: {required: true, email: true},
            ct_user_name: {required: true, number: true, minlength: 10, maxlength: 10},
            //ct_user_pass: {required: true, minlength: password_check.min, maxlength: password_check.max}
        },
        messages: {
            //ct_user_name: {required: errorobj_please_enter_email_address, email: errorobj_please_enter_valid_email_address},
            ct_user_name: {required: 'Please enter your mobile number', number: 'Please enter 10 digit mobile number', minlength: 'Please enter 10 digit mobile number', maxlength: 'Please enter 10 digit mobile number'},
            //ct_user_pass: {required: errorobj_please_enter_password, minlength: errorobj_min_ps, maxlength: errorobj_max_ps}
        }
    });
    
    jQuery('#user_otp_form').validate({
        rules: {
            //ct_user_name: {required: true, email: true},
            ct_user_otp: {required: true, number: true, minlength: 4, maxlength: 4},
            //ct_user_pass: {required: true, minlength: password_check.min, maxlength: password_check.max}
        },
        messages: {
            //ct_user_name: {required: errorobj_please_enter_email_address, email: errorobj_please_enter_valid_email_address},
            ct_user_otp: {required: 'Please enter OTP', number: 'Please enter 4 digit OTP', minlength: 'Please enter 4 digit OTP', maxlength: 'Please enter 4 digit OTP'},
            //ct_user_pass: {required: errorobj_please_enter_password, minlength: errorobj_min_ps, maxlength: errorobj_max_ps}
        }
    });
});
    ...
    jQuery.ajax({
        ...
        success: function (response) {
            if (jQuery.trim(response) == 'logout successful') {
                ...
                jQuery('#login_existing_user').show();
    ...
    jQuery(document).on('keyup', '#ct_postal_code', function (event) {
        ...
        if (postal_code == '') {
            ...
            if (jQuery.inArray(postal_code, get_all_postal_code) !== -1) {
                ...
                jQuery('.postal_code_error').hide();
                jQuery('.postal_code_available').show();
            } else {
                jQuery('.remove_show_error_class').addClass('show-error');
                jQuery('#complete_bookings').addClass('ct_remove_id');
                jQuery(document).on('click', '.ct_remove_id', function () {
