...
$pdf->Text(80, 20, $client_address['address']);
$pdf->Text(80, 25, $client_address['city'], $client_address['state']);
...
if (!empty($booking_addon_details)) {
    ...
    foreach ($booking_addon_details as $booking_addon) {
		...
		$pdf->Text(75 - $pdf->GetStringWidth($booking_addon["adons_item_codes"]), $addondetails_sttpoint, strtoupper($booking_addon['adons_item_codes']));
		$pdf->Text(115 - $pdf->GetStringWidth($booking_addon["addonprice"]), $addondetails_sttpoint, $booking_addon['addonprice']);
		$pdf->Text(143 - $pdf->GetStringWidth($booking_addon["adons_item_gst"]), $addondetails_sttpoint, $booking_addon['adons_item_gst']);
		$pdf->Text(159 - $pdf->GetStringWidth($booking_addon["adons_tax_percent"]), $addondetails_sttpoint, $booking_addon['adons_tax_percent']."%");
        $pdf->Text(194 - $pdf->GetStringWidth($booking_addon["adons_item_total"]), $addondetails_sttpoint, $booking_addon['adons_item_total']);
...
if (mysqli_num_rows($res_xtra_reqmnts) > 0) {
	...
	$row_xtra_code = strtoupper($row_xtra_reqmnts->item_code_type);
    ...
    $pdf->Text(30, $addondetails_sttpoint, 'Extra Requirements');
	$pdf->Text(75 - $pdf->GetStringWidth($row_xtra_code), $addondetails_sttpoint, $row_xtra_code);
	$pdf->Text(115 - $pdf->GetStringWidth($row_xtra_prices), $addondetails_sttpoint, $row_xtra_prices);
	$pdf->Text(143 - $pdf->GetStringWidth($row_xtra_gst), $addondetails_sttpoint, $row_xtra_gst);
	$pdf->Text(159 - $pdf->GetStringWidth($row_xtra_taxs), $addondetails_sttpoint, $row_xtra_taxs."%");
    $pdf->Text(194 - $pdf->GetStringWidth($row_xtra_total), $addondetails_sttpoint, $row_xtra_total);
}
...
$pdf->Text(165 - $pdf->GetStringWidth($label_language_values['sub_total']), $addondetails_sttpoint + 40, $label_language_values['sub_total']);
if ($payinfo[11] == 'O') {
    $fd = "Once";
} elseif ($payinfo[11] == 'W') {
    $fd = "Weekly";
...
$pdf->Text(131,$addondetails_sttpoint + 20,"Price"); 
$pdf->Text(156,$addondetails_sttpoint + 20,"Total GST"); 
/*$pdf->Text(156,$addondetails_sttpoint + 20,"TAX"); 
$pdf->Text(150, 107, $label_language_values['qty']);
 $pdf->Text(150,107,""); */
$pdf->Text(180, $addondetails_sttpoint + 20, $label_language_values['price']);

$pdf->SetDrawColor(0,0,0);
$pdf->Line(0, $addondetails_sttpoint + 25 , 210, $addondetails_sttpoint + 25); // 20mm from each edge
...
$actual_tax  =  $general->ct_price_format_for_pdf($payment_taxes + $row_xtra_tax, $symbol_position, $decimal);
$net_amount  =  $general->ct_price_format_for_pdf($payment_net_amount + $row_xtra_totals, $symbol_position, $decimal);

$pdf->SetFont('DejaVu', '', 10);
$pdf->Text(131,$addondetails_sttpoint + 31, $actual_amount);
$pdf->Text(156,$addondetails_sttpoint + 31, $actual_tax);
$pdf->Text(180,$addondetails_sttpoint + 31, $net_amount);
...
$pdf->Text(150,$addondetails_sttpoint + 45,"Total GST"); 
//$pdf->Text(165 - $pdf->GetStringWidth($label_language_values['tax']), $addondetails_sttpoint + 50, $label_language_values['tax']);



$pdf->SetFont('DejaVu', '', 10);
$pdf->Text(200 - $pdf->GetStringWidth($printamount), $addondetails_sttpoint + 40, $printamount);
...
$pdf->Text(200 - $pdf->GetStringWidth($printtaxes), $addondetails_sttpoint + 45, $printtaxes);

$pdf->SetFont('DejaVu', '', 13);

$pdf->Text(165 - $pdf->GetStringWidth($label_language_values['total']), $addondetails_sttpoint + 50, $label_language_values['total']);
$pdf->Text(198 - $pdf->GetStringWidth($printtaxes), $addondetails_sttpoint + 50, $actual_amount);
...
$file = dirname(dirname(__FILE__)) . '/admin/pdf/yupserve' . '-' . $invoice_number . '.pdf';
