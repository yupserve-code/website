...
class cleanto_adminprofile {
    ...
    public function update_password(){
        $query = "UPDATE ct_admin_info SET password='".md5($this->password)."'  where `id`='".$this->id."'";
        $result=mysqli_query($this->conn,$query);
        return $result;
...
//        }
	...
	public function add_staff(){
		$query="insert into `".$this->tablename."` (`id`, `password`, `email`, `fullname`, `phone`, `address`, `city`, `state`, `zip`, `country`,`role`, `description`, `enable_booking`, `service_commission`, `commision_value`, `schedule_type`, `image`, `service_ids`) values(NULL,'".md5($this->pass)."','".$this->email."','".$this->fullname."','" . $this->phone . "', '', '', '', '', '', '".$this->role."', '', 'N', 'F', '0', 'W', '', '')";
		$result=mysqli_query($this->conn,$query);	
	...
	public function check_staff_email_existing(){
        ...
        /*display staff service details  in staff page*/
	public function check_current_staff_email_exists(){
            $query="SELECT COUNT(`id`) AS `c` FROM `".$this->tablename."` WHERE `email`='".$this->email."' AND id='". $this->id ."'";
        $result=mysqli_query($this->conn,$query);
	$value = mysqli_fetch_array($result);
        return $value[0];
	}
        ...
        /*display staff service details  in staff page*/
	public function check_staff_mobile_existing(){
		$query="select count(`id`) as `c` from `".$this->tablename."` where `phone`='".$this->phone."'";
        $result=mysqli_query($this->conn,$query);
	$value = mysqli_fetch_array($result);
        return $value[0];
	}
